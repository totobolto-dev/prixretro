#!/usr/bin/env python3
"""
Scrape current Nintendo DS listings (FOR SALE) with images
Saves to current_listings_ds.json
"""
...
def load_ds_variants():
    """Load Nintendo DS variant config"""
    ...
    if os.path.exists('config_multiconsole.json'):
    ...
    elif os.path.exists('scraped_data_ds.json'):
        # Extract variants from scraped data
        with open('scraped_data_ds.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        variants = {}
        ...
        print("⚠️  No Nintendo DS config found, using default variants")
        # Default Nintendo DS variants
        return {
...
def matches_ds_variant(title, variant_key):
    """Check if listing title matches the Nintendo DS variant"""
    title_lower = title.lower()

    # Must contain "nintendo ds", "gameboy advance", or "ds"
    if not ('nintendo ds' in title_lower or 'gameboy advance' in title_lower or 'ds' in title_lower):
        return False

    # Variant matching logic for Nintendo DS
    variant_keywords = {
        # Standard Nintendo DS
...
def scrape_ds_current_listings(variant_key, variant_config, max_items=15):
    """Scrape active eBay listings for a Nintendo DS variant with images"""

    variant_name = variant_config.get('name', variant_key.replace('-', ' ').title())
    print(f"\n🔍 Scraping current Nintendo DS listings for: {variant_name}")

    # Build search URL for Nintendo DS items FOR SALE
    ...
    try:
        ...
        print(f"  ✅ Found {len(cards)} Nintendo DS cards")
        ...
        for card in cards:
            try:
                ...
                if not matches_ds_variant(title, variant_key):
                    continue
...
def scrape_all_ds_variants():
    """Scrape current listings for all Nintendo DS variants"""

    print("="*70)
    print("🎮 SCRAPING Nintendo DS CURRENT LISTINGS")
    print("="*70)

    variants = load_ds_variants()
    print(f"\n📊 Loaded {len(variants)} Nintendo DS variants")

    all_listings = {}

    for variant_key, variant_config in variants.items():
        listings = scrape_ds_current_listings(variant_key, variant_config, max_items=15)
    ...
    # Save to JSON
    output_file = 'current_listings_ds.json'
