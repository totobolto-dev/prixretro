...
def load_scraped_data():
    """Load GBA scraped data from JSON file and transform to expected format"""
    data_file = 'scraped_data_gba.json'

    if not os.path.exists(data_file):
        print(f"❌ Error: {data_file} not found!")
        print("Please run process_gba_for_launch.py first.")
        return None

    print(f"📂 Loading GBA data from: {data_file}")
    ...
    transformed_data = {}
    for variant_key, variant_info in raw_data.items():
        # Get description from config if available
        description = f"Game Boy Advance {variant_info['name']}"
        if variant_key in config['variants']:
            description = config['variants'][variant_key].get('description', description)

        transformed_data[variant_key] = {
            'variant_key': variant_key,
            'variant_name': variant_info['name'],
            'description': description,
            'stats': {
                'avg_price': variant_info['avg_price'],
                'min_price': variant_info['min_price'],
                'max_price': variant_info['max_price'],
                'listing_count': variant_info['count']
            },
            'items': variant_info['items']
        }

    return transformed_data

def load_config():
    """Load GBA configuration from multi-console config"""
    with open('config_multiconsole.json', 'r', encoding='utf-8') as f:
        full_config = json.load(f)

    # Extract GBA-specific config
    gba_config = {
        'ebay_partner': full_config['ebay_partner'],
        'scraping': full_config['scraping'],
        'variants': full_config['consoles']['game-boy-advance']['variants']
    }
    return gba_config
...
def generate_variant_page(variant_data, all_variants, config, template, output_dir='output', current_listings_data=None):
    ...
    print(f"  📄 Generating: game-boy-advance-{variant_key}.html")
    ...
    for key in all_variants.keys():
        if key != variant_key:
            ...
            related_variants.append(
                f'<a href="/game-boy-advance-{key}.html" class="related-link">{name}</a>'
            )

    related_html = "\n                    ".join(related_variants[:8])

    # Replace placeholders (Laravel-ready structure)
    product_name = f"Game Boy Advance {variant_name}"
    ...
    html = html.replace('{PRODUCT_SLUG}', f"game-boy-advance-{variant_key}")
    ...
    filename = f"game-boy-advance-{variant_key}.html"
...
def generate_all_pages():
    ...
    for variant_key, variant_data in scraped_data.items():
        ...
        if not variant_data['items'] or len(variant_data['items']) == 0:
